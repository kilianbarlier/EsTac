...
#'
#' Function to compute the R2, R2 adjusted, AIC or BIC.
#'
#' @param x a numeric vector.
#' @param y a numeric vector.
#' @param para a vector of parameters.
#' @param FUN a function.
#' @param metric a character string indicating which metrics to compute. One of "R2" (default), "R2a", "AIC" or "BIC").
#'
#' @return compute_metric() returns the chosen metrics.
#' @export
#'
#' @examples
compute_metric <- function(x, y, para, FUN, metric = "R2"){
...
estap <- function(age, perf, FUN, nbpara, niter = 10, borne = -Inf,
  ...
  R2 <- compute_metric(age, perf, best_param, FUN, "R2")
  R2a <- compute_metric(age, perf, best_param, FUN, "R2a")
  AIC <- compute_metric(age, perf, best_param, FUN, "AIC")
  BIC <- compute_metric(age, perf, best_param, FUN, "BIC")
